  ...
  <section id="top" class="screen landing">
    <div class="landing-hero">
      ...
      <div class="subtitle">
        ...
        <span class="quote-meaning">Menguasai bahasa Inggris akan membuka jauh lebih banyak kesempatan baru yang sebelumnya tertutup rapat.</span>
      </div>
    </div>
    ...
    <div class="card about-card"><div class="about-grid"><div class="about-text"><h3>Siapa Kami?</h3><p><b>English Club UPB</b> adalah bagian dari <b>BEM Departemen Keilmuan</b>. Wadah yang menciptakan lingkungan bahasa Inggris yang menyenangkan untuk menyalurkan bakat dan minat.</p><p>Tidak perlu jago dulu — belajar bareng, santai, dan aplikatif sebagai bekal karir.</p><ul class="check-list"><li>Bagian dari BEM — Departemen Keilmuan</li><li>Terbuka semua prodi & angkatan</li></ul></div><div class="about-visual"><div class="mini-stats"><div class="mini-stat"><b>2019</b><span>Berdiri</span></div><div class="mini-stat"><b>150+</b><span>Alumni</span></div><div class="mini-stat"><b>4</b><span>Divisi</span></div></div><div class="quote-box">From Kebumen to the World, Speak Confidently, Lead Globally.</div></div></div></div>
    ...
    <div class="overview-grid vision-mission-grid"><article class="overview-card"><div class="ov-icon">01</div><h3>Visi</h3><ul><li>Menciptakan lingkungan bahasa Inggris yang menyenangkan sebagai wadah pengembangan kemampuan, bakat, dan minat.</li><li>Menjadikan bahasa Inggris sebagai bahasa umum untuk menumbuhkan rasa percaya diri dan mempersiapkan mahasiswa menghadapi dunia kerja.</li></ul></article><article class="overview-card"><div class="ov-icon">02</div><h3>Misi</h3><ul><li>Menumbuhkan keberanian dan kepercayaan diri anggota dalam berbahasa Inggris dengan serangkaian program rutin</li><li>Meningkatkan kemampuan bahasa inggris di lingkungan mahasiswa sebagai bekal dalam persaingan dunia karir</li><li>Mengadakan kegiatan berbasis bahasa Inggris</li><li>Membawa UKM English Club sebagai wadah penggerakan kualitas yang dibutuhkan dalam aspek kegiatan khususnya dilingkungan yang Berkaitan</li></ul></article></div>

    <div class="program-section"><h2>Program Kerja</h2><p class="tiny muted" style="margin-bottom:14px;">Klik card untuk lihat penjelasan & foto (gallery 3).</p><div class="program-grid"><article v-for="p in proker" :key="p.id" class="program-card clickable" role="button" tabindex="0" @click="openProker(p)" @keydown.enter="openProker(p)"><div class="program-thumb" v-if="p.photos && p.photos.length"><img :src="p.photos[0]" :alt="p.title" loading="lazy" /></div><div v-else class="program-icon">{{ String(p.order).padStart(2,'0') }}</div><h3>{{ p.title }}</h3><p>{{ p.caption }}</p><span class="tiny" style="margin-top:6px; font-weight:800; color:var(--royal-blue);">Lihat detail →</span></article></div></div>
    <ProgramModal :program="selectedProker" :open="!!selectedProker" @close="closeProker" />

  <div class="teaser-card"><div class="teaser-mark">5x5</div><div class="teaser-copy"><span class="teaser-kicker">MINI GAME ENGLISH CLUB</span><h2>Ayo Main <span>Word Hunt</span></h2><p>Latih vocabulary dengan game swipe buatan EC UPB. Susun kata bahasa Inggris, kumpulkan poin, dan tantang temanmu.</p></div><div class="teaser-actions"><button class="btn" type="button" @click="goPlay">Mulai Bermain</button><button class="btn ghost" type="button" @click="goBoard">Lihat Klasemen</button></div></div>
...
.story-section { width:100%; padding:86px 0 0; }
