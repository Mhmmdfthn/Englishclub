...
const filteredProker = computed(() => {
  ...
  if (activeFilter.value === 'upcoming') return proker.value.filter(p => (p.status || 'upcoming') === 'upcoming')
  if (activeFilter.value === 'ongoing') return proker.value.filter(p => p.status === 'ongoing')
  return proker.value.filter(p => p.status === 'completed')
})
const prokerCounts = computed(() => ({
  all: proker.value.length,
  upcoming: proker.value.filter(p => (p.status || 'upcoming') === 'upcoming').length,
  ongoing: proker.value.filter(p => p.status === 'ongoing').length,
  completed: proker.value.filter(p => p.status === 'completed').length,
}))
const PROKER_PAGE = 6
...
async function submitStory() {
  ...
  try {
    ...
    storyBatch.value = ''
    storyComment.value = ''
  } catch { storyError.value = 'Kesan belum dapat dikirim. Coba lagi.' }
...
<template>
  <section id="top" class="screen landing">
    ...
    <div class="program-section"><div class="program-header"><div><h2>Program Kerja</h2><p>Temukan kegiatan untuk belajar, bermain, dan berkembang bersama English Club.</p></div><div class="program-filters"><button class="filter-btn" :class="{ active: activeFilter === 'all' }" @click="activeFilter = 'all'">Semua<span class="count-badge">{{ prokerCounts.all }}</span></button><button class="filter-btn" :class="{ active: activeFilter === 'upcoming', 'upcoming-active': activeFilter === 'upcoming' }" @click="activeFilter = 'upcoming'">Mendatang<span class="count-badge">{{ prokerCounts.upcoming }}</span></button><button class="filter-btn" :class="{ active: activeFilter === 'ongoing', 'ongoing-active': activeFilter === 'ongoing' }" @click="activeFilter = 'ongoing'">Berlangsung<span class="count-badge">{{ prokerCounts.ongoing }}</span></button><button class="filter-btn" :class="{ active: activeFilter === 'completed', 'completed-active': activeFilter === 'completed' }" @click="activeFilter = 'completed'">Selesai<span class="count-badge">{{ prokerCounts.completed }}</span></button></div></div><div class="program-grid"><article v-for="p in displayedProker" :key="p.id" class="program-card clickable" role="button" tabindex="0" @click="emit('goArticle', p)" @keydown.enter="emit('goArticle', p)"><div class="program-thumb"><img :src="p.imageUrl || p.photos?.[0] || '/Logo_ec.jpg'" :alt="p.title" loading="lazy" /><div class="program-status" :class="`status-${p.status || 'upcoming'}`">{{ p.status === 'completed' ? 'Selesai' : p.status === 'ongoing' ? 'Sedang Berlangsung' : 'Akan datang' }}</div></div><div class="program-card-content"><h3>{{ p.title }}</h3><p>{{ textOnly(p.description || p.caption).slice(0, 160) }}</p><div class="program-meta"><span v-if="p.date" class="meta-item"><span class="material-symbols-outlined" style="font-size:15px;vertical-align:middle;margin-right:2px;">calendar_month</span>{{ p.date }}</span><span class="meta-item"><span class="material-symbols-outlined" style="font-size:15px;vertical-align:middle;margin-right:4px;">location_on</span>Universitas Putra Bangsa</span></div><a href="#" class="program-detail-link" @click.stop.prevent="emit('goArticle', p)">Lihat detail →</a></div></article></div><div v-if="hasMoreProker" class="program-more"><button class="filter-btn" type="button" @click="toggleShowAll">{{ showAll ? 'Lihat lebih sedikit ▲' : 'Lihat semua ▼' }}</button></div></div>
...
.program-section { width:100%; margin-top:64px; padding-bottom:24px; border-bottom:3px solid var(--ink); }.program-header { display:flex; align-items:start; justify-content:space-between; gap:24px; margin-bottom:28px; flex-wrap:wrap; }.program-header > div:first-child { flex:1; min-width:300px; }.program-section h2 { font-size:clamp(22px,4vw,28px); font-weight:900; color:var(--ink); margin-bottom:8px; }.program-section > .program-header > div:first-child p { font-size:15px; color:var(--ink); opacity:0.8; }.program-filters { display:flex; flex-wrap:wrap; gap:8px; }.filter-btn { display:inline-flex; align-items:center; gap:6px; padding:9px 14px; color:var(--ink); background:#fff; border:1.5px solid var(--ink); font-family:'Outfit',sans-serif; font-weight:700; font-size:13px; cursor:pointer; transition:transform 0.12s ease, background 0.2s ease, color 0.2s ease, border-color 0.2s ease, box-shadow 0.2s ease; }
.filter-btn:active { transform:scale(0.96); }.filter-btn .count-badge { display:inline-flex; align-items:center; justify-content:center; min-width:20px; height:20px; padding:0 6px; background:var(--ink); color:#fff; border-radius:999px; font-size:11px; font-weight:800; }.filter-btn.active .count-badge { background:rgba(255,255,255,0.28); }.filter-btn.active { color:#fff; background:var(--royal-blue); border-color:var(--royal-blue); box-shadow:0 3px 0 var(--ink); }
.filter-btn.active.upcoming-active { background:#2563eb; border-color:#2563eb; }
.filter-btn.active.ongoing-active { color:#000; background:#F4C542; border-color:#F4C542; }
.filter-btn.active.completed-active { background:#16a34a; border-color:#16a34a; }.filter-btn:focus { outline:none; }@media (hover: hover) { .filter-btn:hover { background:rgba(33,76,122,0.08); } }.program-more { display:flex; justify-content:center; margin-top:24px; }.program-more .filter-btn { padding:10px 22px; }.program-grid { width:100%; display:grid; grid-template-columns:repeat(3,1fr); gap:24px; }.program-card { display:flex; flex-direction:column; background:#fff; border:1px solid #d1d5db; cursor: pointer; transition:transform 0.2s ease, box-shadow 0.2s ease; }.program-card.clickable:hover { transform:translateY(-4px); box-shadow:0 8px 16px rgba(29,43,58,0.15); } .program-thumb { position:relative; width:100%; aspect-ratio: 4/3; border-bottom:1px solid #e5e7eb; overflow:hidden; background:#f0f4f8; } .program-thumb img { width:100%; height:100%; object-fit:cover; display:block; } .program-status { position:absolute; top:12px; left:12px; padding:5px 10px; background:var(--vibrant-yellow); color:#000; border:none; font-size:11px; font-weight:800; } .program-card-content { display:flex; flex-direction:column; gap:12px; padding:16px; flex:1; } .program-card h3 { margin:0; font-size:16px; font-weight:900; color:var(--ink); line-height:1.3; }.program-card p { margin:0; font-size:13px; line-height:1.5; color:var(--ink); display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }.program-meta { display:flex; flex-direction:column; gap:6px; font-size:12px; color:var(--ink); opacity:0.7; margin-top:auto; }.meta-item { display:flex; align-items:center; gap:6px; }.program-detail-link { color:var(--royal-blue); font-weight:700; font-size:13px; text-decoration:none; cursor:pointer; margin-top:8px; }
...
@media (max-width:680px) {
  ...
  .program-card-content { padding:12px; }
  .program-card h3 { font-size:18px; }
  .program-card p { font-size:15px; }
  .program-header { flex-direction:column; gap:14px; margin-bottom:20px; }
  .program-header > div:first-child { min-width:100%; }
  .program-filters { display:grid; grid-template-columns:1fr 1fr; width:100%; }
  .filter-btn { justify-content:center; width:100%; min-height:46px; padding:10px 12px; font-size:13px; }
...
.program-detail-link { margin-top:auto; width:100%; padding-top:12px; border-top:1px solid #e5e7eb; display:flex; align-items:center; justify-content:space-between; font-size:14px; font-weight:800; letter-spacing:.02em; color:var(--royal-blue); }
.status-upcoming { color:#1e40af; background:#dbeafe; }
.status-ongoing { color:#000; background:#F4C542; animation: status-pulse 1.6s ease-in-out infinite; }
.status-completed { color:#fff; background:#22c55e; border-color:#15803d; }
@keyframes status-pulse {
  0%, 100% { box-shadow: 0 0 0 0 rgba(244, 197, 66, 0.55); }
