...
<template>
  <div class="rte">
    <div class="rte-toolbar">
      <button type="button" title="Tebal" :class="{ on: editor?.isActive('bold') }" @click="editor?.chain().focus().toggleBold().run()"><span class="ms material-symbols-outlined">format_bold</span></button>
      <button type="button" title="Miring" :class="{ on: editor?.isActive('italic') }" @click="editor?.chain().focus().toggleItalic().run()"><span class="ms material-symbols-outlined">format_italic</span></button>
      <button type="button" title="Garis bawah" :class="{ on: editor?.isActive('underline') }" @click="editor?.chain().focus().toggleUnderline().run()"><span class="ms material-symbols-outlined">format_underlined</span></button>
      <span class="rte-sep"></span>
      <button type="button" title="Daftar poin" :class="{ on: editor?.isActive('bulletList') }" @click="editor?.chain().focus().toggleBulletList().run()"><span class="ms material-symbols-outlined">format_list_bulleted</span></button>
      <button type="button" title="Daftar angka" :class="{ on: editor?.isActive('orderedList') }" @click="editor?.chain().focus().toggleOrderedList().run()"><span class="ms material-symbols-outlined">format_list_numbered</span></button>
      <span class="rte-sep"></span>
      ...
      <span class="rte-sep"></span>
      <button type="button" title="Rata kiri" :class="{ on: editor?.isActive({ textAlign: 'left' }) }" @click="editor?.chain().focus().setTextAlign('left').run()"><span class="ms material-symbols-outlined">format_align_left</span></button>
      <button type="button" title="Rata tengah" :class="{ on: editor?.isActive({ textAlign: 'center' }) }" @click="editor?.chain().focus().setTextAlign('center').run()"><span class="ms material-symbols-outlined">format_align_center</span></button>
      <button type="button" title="Rata kanan" :class="{ on: editor?.isActive({ textAlign: 'right' }) }" @click="editor?.chain().focus().setTextAlign('right').run()"><span class="ms material-symbols-outlined">format_align_right</span></button>
      <button type="button" title="Rata penuh" :class="{ on: editor?.isActive({ textAlign: 'justify' }) }" @click="editor?.chain().focus().setTextAlign('justify').run()"><span class="ms material-symbols-outlined">format_align_justify</span></button>
    </div>
...
.rte-sep { flex: 0 0 2px; width: 2px; height: 22px; margin: 0 2px; background: rgba(29,43,58,0.2); }
.rte-editor { min-height: 120px; }
.rte-editor :deep(.ProseMirror) { min-height: 120px; max-width: 100%; padding: 10px 12px; outline: none; font-size: 14px; line-height: 1.7; color: var(--dark-navy); overflow-wrap: anywhere; }
...
@media (max-width: 680px) {
  ...
  .rte-sep { height: 18px; }
  .rte-editor,
  .rte-editor :deep(.ProseMirror) { min-height: 76px; }
  .rte-editor :deep(.ProseMirror) { padding: 8px 10px; line-height: 1.5; }
}
</style>